...
//    howToPlayItem.position = ccp(0, -55);
    CCMenuItem *itemHelp = [CCMenuItemFont itemWithString:NSLocalizedString(@"MORE", @"MORE") block:^(id sender){
        [self showAbout];
    }];
     itemHelp.position = ccp(isPad?-200:-100, isPad?0:-50);
    
    CCLabelTTF *readme = [CCLabelTTF labelWithString:@"Drag the entire row, column, or swap pairs fruit to create the line of the three identical fruits to eliminate, you can also use help tools" dimensions:CGSizeMake(isPad?600:300,150) alignment:UITextAlignmentCenter fontName:@"Arial" fontSize:isPad?22:14];
    readme.position = ccp(size.width/2, isPad?10:-20);
    [self addChild:readme];
    
    // 创建菜单
    CCMenu* menu = [CCMenu menuWithItems: itemHelp,item,musicItem,soundItem, nil];
    if(isPad) {
